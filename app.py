...
# Path to dataset folder (absolute path)
...
if not os.path.exists(DATASET_DIR):
    ...
    selected_images = []
    cols = st.columns(8)  # show 8 images per row for smaller size
    for idx, img_name in enumerate(image_files):
        with cols[idx % 8]:
            img_path = os.path.join(DATASET_DIR, img_name)
            st.image(img_path, width=120)  # smaller thumbnail size
            if st.checkbox("Like", key=img_name):
                selected_images.append(img_name)

    # Show recommendations when clicked
